...
|icon-datasource-processor| Processor
	:ref:`Processors <processing-and-logic-processor>` are a type of Data Source containing a process that generates multiple output values, which are stored as new Parameters. Processors can be created inside Locations only.
...
|icon-point-number-range-process| Number Process Parameter
	Number :ref:`Process Parameters <processing-and-logic-process-parameter>` are similar to Number Parameters however the value is generated by a process.
...
|icon-point-number-discrete-process| Number Process Parameter (State variation)
	Number :ref:`Process Parameters <processing-and-logic-process-parameter>` can also be set to display the current or historic state rather than value. This is useful for Boolean or Discrete state data.
...
|icon-point-text-process| Text Process Parameter
	Text :ref:`Process Parameters <processing-and-logic-process-parameter>` are similar to Text Parameters however the value is generated by a process.
...
|icon-point-time-process| Time Process Parameter
	Time :ref:`Process Parameters <processing-and-logic-process-parameter>` are similar to Time Parameters however the value is generated by a process.
